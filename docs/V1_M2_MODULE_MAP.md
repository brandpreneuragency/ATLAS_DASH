...
| Control approvals inbox | Control: `frontend/src/pages/Inbox.tsx` | Today | M5 | Functional placement only; DASH `TodayApprovals` is the single final presentation for this domain and Control remains backend owner (D-APPROVALS). Amended by DP-1, 2026-07-24. |
...
| Hermes chat mode, excluding approvals | DASH: `src/components/chatMode/` | Agent | M5 | Single Hermes presentation for session list and chat pane, rewritten onto Control's canonical runs/session API (D-CHAT); the command-approvals popover is mapped separately below. |
| Workflow gate approvals | DASH: `src/components/today/TodayApprovals.tsx` | Today | M5 | The single presentation for **persistent, run-linked** approvals: `approvals` table, `GET /api/approvals`, `POST /api/approvals/{id}/resolve`; resolving one resumes a paused run (D-APPROVALS). Amended by DP-1, 2026-07-24. |
| Hermes command approvals | DASH: `src/components/chatMode/CommandApprovalsPopover.tsx` | Agent | M5 | The single presentation for **ephemeral** Hermes dangerous-command prompts: delivered over WebSocket via `useHermesStore`, answered with `approval.respond { choice }`, no database row and no run to resume. Renamed from `ApprovalsInbox` by DP-1, 2026-07-24. |
...
Left nav: `Today` opens directly to `Approvals`; no second Control inbox route remains. The user first sees the DASH `TodayApprovals` presentation populated from Control's canonical pending-approval records, with approve/deny actions and links back to the relevant Agent session/run. V1 Today is intentionally thin; Q1 in V3 adds due tasks, in-flight runs, and CRM follow-ups without changing approvals ownership.

**Amendment — DP-1, ratified 2026-07-24: there are two approval domains, not one.** This map originally said "the single approvals presentation", which the M5 build could not honestly satisfy, because the code contains two unrelated things that both happen to be called approvals:

1. **Workflow gate approvals** (Today) — rows in the `approvals` table, tied to a `run_id`, served by `/api/approvals`. Resolving one resumes a paused workflow run. This is the domain D-APPROVALS owns and the domain M4's atomic-claim/409 work hardened.
2. **Hermes dangerous-command approvals** (Agent → chat header bell) — ephemeral WebSocket prompts from `useHermesStore`, answered with `approval.respond { choice: "once" | "deny" }`. No database row, no run, nothing to resume. It was never wired to `/api/approvals`.

The data models do not overlap, and merging them would force one component to juggle two incompatible action semantics ("allow this once" vs "resume the run"). Both presentations are therefore kept. The chat-side component was renamed `ApprovalsInbox` → `CommandApprovalsPopover` so that neither reads as "the" approvals inbox — that naming collision is what made this map ambiguous in the first place. D-APPROVALS is unchanged: Control still owns the approvals records and guards for domain 1.
